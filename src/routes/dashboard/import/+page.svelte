<script lang="ts">
	...
	let filteredRows = $derived.by(() => {
		...
		if (showOnlyUnmapped) result = result.filter((r) => r.categoryName === 'Ukendt' || !r.categoryId);
		return result;
	});
				...
				<form
					...
					use:enhance={() => {
						...
						return async ({ result, update }) => {
							...
							else if (result.type === 'failure') resultMessage = (result.data as any)?.error || 'Ukendt fejl';
						};
					}}
					class="space-y-6"
				>
					...
					{#if resultMessage}
						<div class="rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-sm font-medium text-red-700 dark:text-red-300">
							{resultMessage}
						</div>
					{/if}
							...
							{#each filteredRows as row (row.hash)}
								<tr class="border-b border-slate-100 transition-colors hover:bg-slate-50 dark:border-white/5 dark:hover:bg-slate-700/50" class:opacity-50={row.isExisting}>
									...
									<td class="max-w-[300px] p-3">
										<p class="truncate text-xs font-medium text-slate-800 dark:text-slate-200" title={row.text}>
											{row.text}
										</p>
									...
									<td class="whitespace-nowrap p-3 text-right text-xs font-bold text-slate-800 dark:text-slate-200">
										{row.amountFormatted}
									</td>
									<td class="whitespace-nowrap p-3 text-xs">
										{#if row.paidBy === 'Mathilde'}
											<span class="rounded-full bg-pink-100 px-2 py-1 font-bold text-pink-700 dark:bg-pink-900/30 dark:text-pink-300">M</span>
