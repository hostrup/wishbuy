...
{#if showCategoryModal}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
		<div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md p-6 overflow-hidden relative max-h-[90vh] flex flex-col">
			<button onclick={() => {showCategoryModal = false; editCatId = null; isCreatingCat = false;}} class="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:text-slate-300 text-xl font-bold z-10">✕</button>
			...
			{#if editCatId === null && !isCreatingCat}
				<div class="flex-1 overflow-y-auto space-y-2 pr-2 mb-4">
					{#each data.categories as cat}
						<button onclick={() => openCategoryEdit(cat)} class="w-full flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900/50 hover:bg-indigo-50 border border-slate-200 dark:border-white/10 rounded-xl transition-colors text-left group">
							<span class="font-medium text-slate-700 dark:text-slate-200">{cat.icon} {cat.name}</span>
							<span class="text-xs font-bold text-indigo-500 opacity-0 group-hover:opacity-100 transition-opacity">Rediger</span>
						</button>
					{/each}
				</div>
				<button onclick={openCategoryCreate} class="w-full py-3 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors border border-slate-200 dark:border-white/10">
					+ Opret Ny Kategori
				</button>
			{:else if isCreatingCat}
				<form id="createCatForm" method="POST" action="?/createCategory" use:enhance={() => {
					...
					<div>
						<label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Vælg Kategori Ikon</label>
						<div class="flex flex-wrap gap-1.5 max-h-36 overflow-y-auto p-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-xl">
							{#each commonEmojis as emoji}
								<button type="button" onclick={() => editCatIcon = emoji} class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 text-lg transition-all {editCatIcon === emoji ? 'bg-indigo-200 ring-2 ring-indigo-500 shadow-sm' : ''}">
									{emoji}
								</button>
							{/each}
						</div>
					...
					<div>
						<label for="name" class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Kategorinavn</label>
						<input type="text" id="name" name="name" bind:value={editCatName} required class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-medium transition-all">
					</div>
				</form>
				<div class="pt-2 flex gap-3 mt-5">
					<button type="button" onclick={() => isCreatingCat = false} class="flex-1 py-3.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition-all">Annuller</button>
			...
			{:else}
				...
				<div class="pt-2 flex gap-2 mt-5">
					<button type="button" onclick={() => editCatId = null} class="flex-1 py-3.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition-all">Annuller</button>
...
{#if showProfileModal}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
		<div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-sm p-6 overflow-hidden relative">
			<button onclick={() => showProfileModal = false} class="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:text-slate-300 text-xl font-bold">✕</button>
			<h3 class="text-xl font-bold text-slate-800 dark:text-white mb-6">Rediger Din Profil</h3>
			...
			<form method="POST" action="?/updateProfile" use:enhance={() => {
				...
				<div>
					<label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Din Personlige Emoji</label>
					<div class="flex flex-wrap gap-1.5 max-h-36 overflow-y-auto p-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-xl">
						{#each commonEmojis as emoji}
							<button type="button" onclick={() => profileEmoji = emoji} class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 text-lg transition-all {profileEmoji === emoji ? 'bg-indigo-200 ring-2 ring-indigo-500 shadow-sm' : ''}">
								{emoji}
							</button>
						{/each}
					</div>
				...
				<div>
					<label for="displayName" class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Dit Viste Navn</label>
					<input type="text" id="displayName" name="displayName" value={data.user?.displayName || data.user?.username || ''} required class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-medium transition-all" placeholder="Fx Ronni Hostrup">
				</div>
...
<div class="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 transition-colors duration-300 p-4 md:p-12 font-sans">
	<div class="max-w-7xl mx-auto space-y-6 md:space-y-8">
		...
		<header class="flex flex-col md:flex-row md:justify-between md:items-end border-b border-slate-200 dark:border-white/10 pb-4 md:pb-6">
			<div>
		...
		<section class="flex overflow-x-auto pb-4 pt-1 snap-x snap-mandatory gap-4 md:grid md:grid-cols-3 lg:grid-cols-5 md:overflow-visible [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
			...
			<div class="w-[85vw] md:w-auto flex-shrink-0 snap-center bg-white dark:bg-slate-800 p-5 md:p-6 rounded-2xl shadow-sm border border-indigo-100 flex flex-col justify-center bg-gradient-to-br from-white to-indigo-50/50">
			...
			<a href="/dashboard" class="w-[85vw] md:w-auto flex-shrink-0 snap-center bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50 transition-colors p-5 md:p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 flex flex-col justify-center group cursor-pointer relative overflow-hidden">
				...
				<p class="text-3xl font-black text-slate-800 dark:text-white">{formatCur(data.kpis.currentMonthExpenses || 0)}</p>
				<p class="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium truncate max-w-[150px]">Største post: {data.kpis.topCategoryName || 'Ingen'}</p>
			</a>

			<div class="w-[85vw] md:w-auto flex-shrink-0 snap-center bg-white dark:bg-slate-800 p-5 md:p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 flex items-center gap-4">
				...
				<div class="flex flex-col justify-center">
					...
					<div class="text-[11px] text-slate-600 dark:text-slate-300 font-medium flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-indigo-500"></span> Drømme ({Math.round(wishVsBuyPct)}%)</div>
					<div class="text-[11px] text-slate-600 dark:text-slate-300 font-medium flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-slate-300"></span> Købt ({100 - Math.round(wishVsBuyPct)}%)</div>
				</div>
			</div>
			...
			<div class="w-[85vw] md:w-auto flex-shrink-0 snap-center bg-white dark:bg-slate-800 p-5 md:p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 flex items-center gap-4">
				...
				<div class="flex flex-col justify-center">
					...
					<div class="text-[11px] text-slate-600 dark:text-slate-300 font-medium flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-violet-500"></span> Fælles ({Math.round(sharedPct)}%)</div>
					<div class="text-[11px] text-slate-600 dark:text-slate-300 font-medium flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-rose-500"></span> Ego ({100 - Math.round(sharedPct)}%)</div>
				</div>
			</div>
			...
			<div class="w-[85vw] md:w-auto flex-shrink-0 snap-center bg-white dark:bg-slate-800 p-5 md:p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 flex items-center gap-4">
				...
				<div class="flex flex-col justify-center">
					...
					<div class="text-[11px] text-slate-600 dark:text-slate-300 font-medium flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-emerald-500"></span> Fælles ({Math.round(buySharedPct)}%)</div>
					<div class="text-[11px] text-slate-600 dark:text-slate-300 font-medium flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-amber-500"></span> Ego ({100 - Math.round(buySharedPct)}%)</div>
				</div>
			</div>

			<div class="w-[85vw] md:w-auto flex-shrink-0 snap-center bg-white dark:bg-slate-800 p-5 md:p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 flex flex-col justify-between">
				<div class="mb-2 md:mb-0">
					<p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Største Drømmer</p>
					<p class="text-sm font-bold text-slate-800 dark:text-white">{data.kpis.topDreamer.name} <span class="font-normal text-slate-500 dark:text-slate-400 text-xs ml-1">({formatCur(data.kpis.topDreamer.amount)})</span></p>
				</div>
				<div>
					<p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Største Forbruger</p>
					<p class="text-sm font-bold text-slate-800 dark:text-white">{data.kpis.topSpender.name} <span class="font-normal text-slate-500 dark:text-slate-400 text-xs ml-1">({formatCur(data.kpis.topSpender.amount)})</span></p>
				</div>
			</div>

			{#if data.kpis.cooldownGain !== undefined}
			<div class="w-[85vw] md:w-auto flex-shrink-0 snap-center bg-white dark:bg-slate-800 p-5 md:p-6 rounded-2xl shadow-sm border border-emerald-100 flex flex-col justify-center bg-gradient-to-br from-white to-emerald-50/50 relative overflow-hidden">
		...
		<main class="grid grid-cols-1 xl:grid-cols-3 gap-6 md:gap-8">
			<section class="xl:col-span-1 order-first">
				<div class="bg-white dark:bg-slate-800 p-5 md:p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 sticky top-4 md:top-6 z-10">
					<h2 class="text-lg font-bold mb-4 border-b border-slate-100 dark:border-white/5 pb-2">Kast i Brønden</h2>
					...
					<form method="POST" action="?/createItem" use:enhance class="space-y-4">
						...
						<div>
							<label for="url" class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Link (valgfrit)</label>
							<input type="url" id="url" name="url" class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-900 dark:bg-slate-800 outline-none transition-all text-sm" placeholder="https://...">
						</div>

						<div>
							<label for="price" class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Pris (DKK)</label>
							<input type="number" step="0.01" id="price" name="price" required class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-900 dark:bg-slate-800 outline-none transition-all font-bold text-lg" placeholder="0">
						</div>

						<div class="grid grid-cols-2 gap-3">
							<div>
								<div class="flex justify-between items-center mb-1">
									<label for="categoryId" class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Kategori</label>
								...
								<select id="categoryId" name="categoryId" required class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-medium">
						...
						<div class="bg-slate-50 dark:bg-slate-900/50 p-3 border border-slate-200 dark:border-white/10 rounded-xl mt-2">
							...
							<p class="text-[10px] text-slate-500 dark:text-slate-400 font-medium text-center mt-2 h-4">{desireLabels[newDesireLevel-1]}</p>
						</div>
						...
						<div class="pt-2 flex flex-col gap-2">
							...
							<button type="submit" name="targetStatus" value="PURCHASED" class="w-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold py-3 rounded-xl hover:bg-emerald-100 hover:text-emerald-700 active:scale-[0.98] transition-all text-sm">
			...
			<section class="xl:col-span-2 space-y-8 md:space-y-10 order-last">
				...
				<div>
					<h2 class="text-xl font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">✨ I Brønden <span class="text-sm font-medium bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full">{data.wishes.length}</span></h2>
					
					{#if data.wishes.length === 0}
						<div class="p-8 text-center bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-white/10 border-dashed text-slate-500 dark:text-slate-400">Brønden er tom.</div>
					{/if}
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
						{#each data.wishes as item}
							...
							<div class="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-white/10 flex flex-col hover:border-indigo-300 transition-colors relative group">
								...
								<div class="flex justify-between items-start mb-3">
									...
									<div class="flex gap-1 bg-slate-50 dark:bg-slate-900/50 p-1 rounded-lg">
								...
								<h3 class="text-lg md:text-xl font-bold text-slate-800 dark:text-white leading-tight pr-4">{item.title}</h3>
								...
								<div class="flex justify-between items-center mt-1 mb-2">
									<div class="text-[11px] font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1">
										...
										<form method="POST" action="?/changeItemUser" use:enhance class="inline-block relative group/user">
											<input type="hidden" name="itemId" value={item.id} />
											<select name="userId" onchange={(e) => e.currentTarget.form?.requestSubmit()} class="appearance-none bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300 font-bold px-1.5 py-0.5 pr-4 rounded cursor-pointer outline-none transition-colors border-none text-[11px]">
								...
								<div class="flex gap-2 pt-4 border-t border-slate-100 dark:border-white/5 mt-auto">
									{#if item.url}
										<a href={item.url} target="_blank" rel="noopener noreferrer" class="p-3 md:p-2 bg-slate-50 dark:bg-slate-900/50 rounded-xl md:rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 text-center flex-shrink-0 transition-colors">🔗</a>
									{/if}
									...
									<form method="POST" action="?/toggleStatus" use:enhance onsubmit={(e) => {
										...
										<input type="hidden" name="itemId" value={item.id} />
										<button type={isLocked ? 'button' : 'submit'} class="w-full text-sm font-bold py-3 md:py-2 rounded-xl md:rounded-lg transition-all {isLocked ? 'bg-slate-100 dark:bg-slate-700 text-slate-400 cursor-not-allowed border border-slate-200 dark:border-white/10' : 'bg-emerald-50 border border-emerald-100 hover:bg-emerald-500 hover:text-white text-emerald-600 active:scale-[0.98]'}">
											{#if isLocked}
				...
				<div class="opacity-90">
					<h2 class="text-lg font-bold text-slate-500 dark:text-slate-400 mb-4 flex items-center gap-2">💸 Realiseret (Købt) <span class="text-xs font-medium bg-slate-200 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full">{data.purchases.length}</span></h2>
					
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
						{#each data.purchases as item}
							<div class="bg-slate-100 dark:bg-slate-700/70 p-4 md:p-5 rounded-2xl border border-slate-200 dark:border-white/10 flex flex-col relative group">
								...
								<div class="absolute -top-2 -right-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-10">
									<form method="POST" action="?/deleteItem" use:enhance onsubmit={(e) => { if (!confirm('Slet denne post fra historikken?')) e.preventDefault(); }}>
										<input type="hidden" name="itemId" value={item.id} />
										<button class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 hover:bg-red-500 text-slate-400 hover:text-white w-7 h-7 rounded-full flex items-center justify-center shadow-sm transition-colors text-[10px]" title="Slet">✕</button>
									</form>
								</div>
								...
								<div class="flex justify-between items-start mb-1.5">
									<div class="flex items-center gap-2">
										<form method="POST" action="?/changeItemExpenseType" use:enhance class="inline-flex group/expense">
											<input type="hidden" name="itemId" value={item.id} />
											<div class="relative inline-flex items-center">
												<select name="expenseType" onchange={(e) => e.currentTarget.form?.requestSubmit()} class="appearance-none text-[9px] font-bold px-1.5 py-0.5 pr-3.5 rounded text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 cursor-pointer focus:ring-0 outline-none transition-colors">
													<option value="SHARED" selected={item.expenseType === 'SHARED'}>FÆLLES</option>
													<option value="PERSONAL" selected={item.expenseType === 'PERSONAL'}>EGO</option>
												</select>
								...
								<h3 class="text-base font-semibold text-slate-500 dark:text-slate-400 line-through decoration-slate-400/50 pr-4">{item.title}</h3>
								...
								<div class="flex justify-between items-center mt-1 mb-2">
									<div class="text-[10px] font-medium text-slate-400 flex items-center gap-1">
										...
										<form method="POST" action="?/changeItemUser" use:enhance class="inline-block relative group/user">
											<input type="hidden" name="itemId" value={item.id} />
											<select name="userId" onchange={(e) => e.currentTarget.form?.requestSubmit()} class="appearance-none bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50 text-slate-600 dark:text-slate-300 font-bold px-1.5 py-0.5 pr-4 rounded cursor-pointer outline-none transition-colors text-[10px]">
												{#each data.users as u}
													<option value={u.id} selected={u.id === item.userId}>
														{u.emoji || '👤'} {u.displayName || u.username}
													</option>
